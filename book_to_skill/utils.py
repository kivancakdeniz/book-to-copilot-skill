...
_MARKDOWN_LOCAL_LINK = re.compile(r"^\s*[-*+]\s+\[[^]]+\]\(#([^)]+)\)\s*$")
...
def _has_markdown_anchor_index(text: str) -> bool:
    """Return whether a Markdown list links to at least three real headings."""
    anchors = {
        match.group(1).casefold()
        for line in text[:30000].splitlines()
        if (match := _MARKDOWN_LOCAL_LINK.match(line))
    }
    if len(anchors) < 3:
        return False
    ...
    return len(anchors & heading_anchors) >= 3
...
def detect_structure(text: str) -> dict:
    ...
    # Look for named ToC headers or a Markdown section index near the front.
    has_toc = bool(_TOC_PATTERN.search(text[:30000])) or _has_markdown_anchor_index(text)
