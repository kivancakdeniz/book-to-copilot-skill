"""Contracts for the shared enterprise skill release factory."""
...
def prepared_root(tmp_path_factory: pytest.TempPathFactory) -> Path:
    return _prepare_valid_root(tmp_path_factory.mktemp("skill-release-root"))
...
def built_release(
    ...
    output = tmp_path_factory.mktemp("skill-release")
...
def test_catalog_scenarios_and_source_skill_counts():
    ...
    scenario_paths = sorted(
        (ROOT / "demos").glob("*/evaluation/scenarios.json")
    )
    skill_markdown = sorted(
        (ROOT / "demos").glob("*/skill/**/*.md")
    )
    ...
    assert sum(len(_scenario_list(path)) for path in scenario_paths) == 144
    assert len(skill_markdown) == 72
...
def test_build_creates_fifty_host_archives_and_manifests(built_release):
    ...
    assert top_manifest["demoCount"] == 12
    assert top_manifest["archiveCount"] == 60
...
def test_package_notices_are_demo_specific(built_release):
    ...
    for slug in release.EXPECTED_SLUGS:
        ...
        assert demo.catalog_entry["en"]["title"] in notices
        ...
        assert other.catalog_entry["en"]["title"] not in notices
...
def test_stage_docs_copies_only_public_release_files(tmp_path: Path, prepared_root: Path):
    ...
    assert target == docs / "downloads" / "skills"
...
def test_site_zip_is_rooted_at_built_site_and_includes_downloads(tmp_path: Path):
    site = tmp_path / "site"
    downloads = site / "downloads" / "skills"
...
def _scorecard(slug: str) -> dict:
    return _json(ROOT / "demos" / slug / "evidence" / "scorecard.json")


def _best(scorecard: dict, condition: str) -> dict:
    return max(
...
def test_every_demo_publishes_a_reproducible_control_and_treatment_run():
    for slug in release.EXPECTED_SLUGS:
        demo = ROOT / "demos" / slug
        stored = _scorecard(slug)
        rebuilt = release.build_scorecard(demo)
        assert stored == rebuilt, f"{slug}: scorecard drifted from a clean recomputation"

        control = _best(stored, "control")
        treatment = _best(stored, "treatment")
        assert control["ruleCitationCount"] == 0, f"{slug}: control cited policy rules"
        assert treatment["ruleCitationCount"] == treatment["ruleCitationTotal"]
        assert treatment["traceScore"] > control["traceScore"]
        for run in stored["runs"]:
            output = demo / run["outputPath"]
            assert output.is_file() and not output.is_symlink()
            assert run["outputSha256"] == hashlib.sha256(output.read_bytes()).hexdigest()


def test_answer_keys_stay_locked_to_the_scenario_and_out_of_the_case_brief():
    for slug in release.EXPECTED_SLUGS:
        demo = ROOT / "demos" / slug
        key = _json(demo / "evaluation" / "answer-key.json")
        scenarios = _scenario_list(demo / "evaluation" / "scenarios.json")
        case = next(item for item in scenarios if item["id"] == key["scenarioId"])

        assert key["decisionClass"] == case["expectedDecision"]
        assert key["recommendedOption"] == case["expectedOption"]
        assert key["requiredRuleIds"] == list(case["requiredRules"])

        brief = (demo / "sources" / "case-brief.md").read_text(encoding="utf-8")
        decision = release.normalize(key["decisionClass"]).strip()
        assert f" {decision} " not in release.normalize(brief), f"{slug}: brief leaks the answer"
...
def test_scorer_matches_citations_regardless_of_case_accent_or_punctuation():
    key = {
        ...
        "demoId": "demo",
        "scenarioId": "S-01",
        "decisionClass": "revise-before-launch",
        "recommendedOption": "separate-notice-and-consent",
        "requiredRuleIds": ["KVK-C01", "KVK-N01"],
        "humanRoute": ["Privacy Counsel"],
        "forbiddenClaims": ["onaylıyorum"],
    }
    answer = (
        "Karar: Revise Before Launch. Seçenek: separate notice and consent.\n"
        "Kurallar: kvk c01 ve `KVK-N01`. Rota: PRIVACY COUNSEL."
    )
    result = score_text(key, answer)
    assert result["traceScore"] == 100
    assert result["gates"]["forbiddenClaims"]["passed"] is True
