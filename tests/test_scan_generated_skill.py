...
def test_scanner_flags_nested_reference_files(tmp_path: Path):
    skill = _write_clean_skill(tmp_path / "nested-reference")
    references = skill / "references" / "decisions"
    ...
    assert any(
        finding.path == "references/decisions/policy.md"
        and finding.rule_id == "prompt.fake_system_prefix"
        for finding in findings
    )


def test_scanner_rejects_symbolic_link_references_directory(tmp_path: Path):
    skill = _write_clean_skill(tmp_path / "symlink-references")
    ...
    try:
        ...
        assert "references must be a real directory" in str(exc)
    else:
        raise AssertionError("symbolic-link references directory should fail closed")
