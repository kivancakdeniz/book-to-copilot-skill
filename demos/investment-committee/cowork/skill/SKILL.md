...
Read both [appraisal method](./references/appraisal-method.md) and [company policy](./references/company-policy.md) before assessing a proposal. Use the [evidence map](./references/evidence-map.md) to cite provenance, the [output schema](./references/output-schema.md) for the response, and the [scenario guide](./references/scenario-guide.md) for missing facts, conflicts, abstention, and what-if requests.
...
9. **Draft the memo.** Use the exact executive memo headings and decision-card format in [output schema](./references/output-schema.md). State missing facts, required human approvers, conditions, mitigations, measures, review dates, and stop/review triggers without inventing them.
...
- Cite scenario facts by document and exact section. Use [evidence map](./references/evidence-map.md) for canonical locations.
