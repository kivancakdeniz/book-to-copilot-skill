...
1. [Kamusal yöntem](./public-method.md): Kontrol kavramı ve ciro
   kaynağı için kısa inceleme soruları ile metadata-only sınırı.
2. [Şirket politikası](./company-policy.md): BKP-1.0 kuralları, beş
   karar sınıfı, üç seçenek ve insan yetkisi.
3. [Kanıt haritası](./evidence-map.md): İşlem olguları, precomputed
   sonuç, teknoloji statüsü ve kapanış boşlukları.
4. [Çıktı şeması](./output-schema.md): Zorunlu başlık, tablo, durum
   değerleri ve yanıt sınırı.
5. [Senaryo rehberi](./scenario-guide.md): Eksik/çatışmalı sonuç,
