#!/usr/bin/env python3
"""Validate and build deterministic enterprise skill releases."""
...
from score_skill_answer import (
    SCORECARD_NAME,
    ScoringError,
    build_scorecard,
    load_answer_key,
...
LOCALE_FIELDS = {"title", "sector", "audience", "oneLineValue"}
LOCALE_KEYS = ("en", "tr")
...
DISCLAIMER = (
    "This package supports training and governance design. It is not legal, "
    "medical, engineering, or regulatory advice. Final decisions and all system "
    "actions remain with authorized humans. \u2014 Bu paket eğitim ve yönetişim "
    "tasarımı içindir; hukuki, tıbbi, mühendislik veya mevzuata uygunluk görüşü "
    "değildir. Nihai kararlar ve bütün sistem eylemleri yetkili insanlarda kalır."
)
RELEASE_ROOT_MARKER = ".skill-release-root"
RELEASE_ROOT_MARKER_BYTES = b"skill-release-root-v1\n"
...
def _validate_answer_key(demo_dir: Path, slug: str, case_brief: str) -> Mapping[str, Any]:
    try:
        answer_key = load_answer_key(demo_dir)
    except ScoringError as exc:
        raise ReleaseError(f"{slug}: {exc}") from exc
    if answer_key["demoId"] != slug:
        raise ReleaseError(f"{slug}: answer key demoId does not match")
    scenarios = _read_json(demo_dir / "evaluation" / "scenarios.json")
    cases = scenarios.get("scenarios", scenarios.get("cases"))
    case = next(
        (
            item
            for item in cases
            if isinstance(item, dict) and item.get("id") == answer_key["scenarioId"]
        ),
        None,
    )
    if case is None:
        raise ReleaseError(f"{slug}: answer key scenario is not in the locked set")
    if answer_key["decisionClass"] != case.get("expectedDecision"):
        raise ReleaseError(f"{slug}: answer key decision class drifted from the scenario")
    if answer_key["recommendedOption"] != case.get("expectedOption"):
        raise ReleaseError(f"{slug}: answer key option drifted from the scenario")
    if list(answer_key["requiredRuleIds"]) != list(case.get("requiredRules") or []):
        raise ReleaseError(f"{slug}: answer key rule ids drifted from the scenario")
    expected_decision = normalize(answer_key["decisionClass"]).strip()
    if f" {expected_decision} " in normalize(case_brief):
        raise ReleaseError(f"{slug}: case brief leaks the expected decision class")
    return answer_key


def _validate_evidence(demo_dir: Path, slug: str) -> Mapping[str, Any]:
    scorecard_path = demo_dir / "evidence" / SCORECARD_NAME
    if scorecard_path.is_symlink() or not scorecard_path.is_file():
        raise ReleaseError(f"{slug}: evidence scorecard is missing")
    stored = _read_json(scorecard_path)
    try:
        rebuilt = build_scorecard(demo_dir)
    except ScoringError as exc:
        raise ReleaseError(f"{slug}: {exc}") from exc
    if stored != rebuilt:
        raise ReleaseError(f"{slug}: evidence scorecard does not match a clean recomputation")
    summary = rebuilt["summary"]
    if summary["treatmentTraceScore"] <= summary["controlTraceScore"]:
        raise ReleaseError(f"{slug}: treatment must beat control on the governance trace")
    if summary["controlRuleCitations"] >= summary["treatmentRuleCitations"]:
        raise ReleaseError(f"{slug}: treatment must cite more policy rules than control")
    return summary
...
def _validate_catalog(root: Path) -> list[Mapping[str, Any]]:
    ...
    if not isinstance(entries, list) or len(entries) != expected_count:
        raise ReleaseError(f"catalog must contain exactly {expected_count} entries")
    ids = [entry.get("id") for entry in entries if isinstance(entry, dict)]
    if tuple(ids) != EXPECTED_SLUGS or len(set(ids)) != expected_count:
    ...
    for entry in entries:
        ...
        for locale_key in LOCALE_KEYS:
            locale = entry.get(locale_key)
            if not isinstance(locale, dict) or set(locale) != LOCALE_FIELDS:
                raise ReleaseError(
                    f"catalog entry {entry.get('id')} has incorrect {locale_key} fields"
                )
            for field in LOCALE_FIELDS:
                if not isinstance(locale[field], str) or not locale[field].strip():
                    raise ReleaseError(
        ...
        for field, expected_article in (
            ...
            if entry.get(field) != expected_article:
                raise ReleaseError(f"catalog entry {entry['id']} {field} is not canonical")
            article = _safe_relative_path(root, entry[field], "catalog article")
            if not article.is_file() or article.is_symlink():
                raise ReleaseError(f"catalog article is missing: {entry[field]}")
            if article.stat().st_size > MAX_ARTICLE_BYTES:
                raise ReleaseError(
                    f"catalog article exceeds {MAX_ARTICLE_BYTES} bytes: {entry[field]}"
                )
    return entries
...
def validate_release(root: Path) -> tuple[ValidatedDemo, ...]:
    ...
    for entry in entries:
        ...
        _validate_answer_key(demo_dir, slug, case_brief)
        evidence = _validate_evidence(demo_dir, slug)
        demos.append(
            ValidatedDemo(
                slug=slug,
                catalog_entry=entry,
...
def _build_demo(
    ...
    manifest = {
        ...
        "id": slug,
        "title": demo.catalog_entry["en"]["title"],
        "titleTr": demo.catalog_entry["tr"]["title"],
        "sourceSkillTreeSha256": demo.source_tree_sha256,
        "archives": archive_records,
        "officialSources": list(demo.official_sources),
        "evaluationEvidence": dict(demo.evidence),
...
def _notice_lines(demo: ValidatedDemo, heading: str) -> list[str]:
    lines = [f"{heading} {demo.catalog_entry['en']['title']}", ""]
...
def _stage_evidence(demo: ValidatedDemo, docs_root: Path) -> None:
    """Publish raw control/treatment answers and the scorecard as site assets."""
    target = docs_root / "assets" / "skills" / demo.slug
    if target.exists():
        shutil.rmtree(target)
    (target / "outputs").mkdir(parents=True)
    evidence = demo.demo_dir / "evidence"
    for path in sorted((evidence / "outputs").glob("*.md")):
        data = _read_bounded_bytes(path, MAX_FILE_BYTES, path.name)
        (target / "outputs" / f"{path.stem}.txt").write_bytes(data)
    shutil.copy2(evidence / SCORECARD_NAME, target / SCORECARD_NAME)
    shutil.copy2(evidence / "runs.json", target / "runs.json")
    screenshots = evidence / "screenshots"
    if screenshots.is_dir():
        (target / "screenshots").mkdir()
        for path in sorted(screenshots.glob("*.png")):
            _read_bounded_bytes(path, MAX_FILE_BYTES, path.name)
            shutil.copy2(path, target / "screenshots" / path.name)
    metadata = evidence / "metadata"
    if metadata.is_dir():
        (target / "metadata").mkdir()
        for path in sorted(metadata.glob("*.json")):
            _read_bounded_bytes(path, MAX_JSON_BYTES, path.name)
            shutil.copy2(path, target / "metadata" / path.name)
...
def stage_docs(root: Path, output_dir: Path, docs_dir: Path) -> Path:
    ...
    target = docs_root / "downloads" / "skills"
    if target.exists():
        shutil.rmtree(target)
    ...
    for demo in demos:
        ...
        _stage_evidence(demo, docs_root)
    for name in (
        "LICENSE.md",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        if args.command == "validate":
            ...
            print(f"Validated {len(demos)} enterprise skill demos.")
