...
## Step 9.5 — Validate and scan the generated skill

Before reporting success, loading the skill in another session, or publishing it, validate the generated `SKILL.md` for GitHub Copilot and then run the advisory security scan:

```bash
SKILL_CONVERTER_ROOT="$(cd "$(dirname "$SCRIPT_PATH")/.." && pwd)"
"$PYTHON_BIN" "$SKILL_CONVERTER_ROOT/tools/validate_skill.py" --lens copilot "$SKILLS_HOME/<skill_name>/SKILL.md"
"$PYTHON_BIN" "$SKILL_CONVERTER_ROOT/tools/scan_generated_skill.py" "$SKILLS_HOME/<skill_name>"
```

If validation exits non-zero, repair the generated skill and rerun both checks. If the scanner exits non-zero, stop and ask a human to review its file/line findings. Do not silently rewrite security findings, and do not load or publish the skill until validation passes and findings are resolved or explicitly accepted.
