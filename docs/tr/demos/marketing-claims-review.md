...
[English](../../MARKETING-CLAIMS-REVIEW-DEMO.md)
...
Bu model eşleşmiş Cowork çiftidir: iki koşu da ayrı görevlerde, cevap anahtarı
içermeyen aynı kampanya özetiyle **Claude Opus 4.8** kullandı.

| Değişen unsur | Yalnız LLM: özet | LLM + skill: özet + beş kaynak |
|---|---|---|
| İş yönü | `evidence-bounded-campaign` seçildi | `evidence-bounded-campaign` seçildi |
| Karar sözleşmesi | Serbest metin: “Yayından önce düzelt” | Kesin sınıf: `approve-with-edits` |
| İddia/açıklama envanteri | Birleştirilmiş altı satır | Ayrı ayrı denetlenebilir yedi satır |
| Politika uygulaması | MCS politikası mevcut değildi | MCS-C01 ile MCS-M01 arası uygulandı |
| Kanıt kaynak izi | Kampanya özeti atfı | PE-01/02/03, MCS kuralları, FTC yöntemi |
| İnsan rotası | Genel hukuki/düzenleyici inceleme | Legal, Compliance, Marketing Director, Product Evidence Owner |
| Yayın kontrolleri | Kısmi; sağlanmamış canlı varlık tetikleyicisi eklendi | MCS-R01 kapıları; MCS-M01 bilinmeyenleri korundu |
| Yanıt sözcük sınırı | 629 sözcük; 700 içinde | 862 sözcük; 700 aşıldı |

**Gözlenen skill değeri:** farklı bir kampanya seçeneği keşfetmedi. Makul bir
içerik incelemesini kesin sınıfı, kural/kanıt eşlemesi, adlandırılmış inceleyicileri
ve açık kontrolleri olan izlenebilir bir yayın kararına dönüştürdü.

!!! note "Bu karşılaştırmanın kanıtlamadığı şey"

    Uygulama istemi skill'i açıkça çağırdı ve şemasını istedi; skill ayrıca
    kontrolde bulunmayan politika ve kanıt kaynaklarını sağladı. Bu, nedensel
    model artışını değil oturum açılmış Cowork deneyimini gösterir. Ayrı üç kollu
    değerlendirme, yalnız senaryo, ham belgeler ve derlenmiş skill'i özdeş
    istemlerle karşılaştıracaktır.
...
[Tam boyutlu kontrol kaydını aç](../../assets/marketing-claims-review/evidence/screenshots/01-control-1-1920x1080.png)

![Çalışma alanında özel skill bulunmayan, yalnız özet kullanan Cowork kontrol yanıtı](../../assets/marketing-claims-review/evidence/screenshots/01-control-1-1920x1080.png)

[Kontrol 1 ham yanıtı](../../assets/marketing-claims-review/evidence/outputs/control-1.txt) ·
[Koşu manifesti](../../assets/marketing-claims-review/evidence/metadata/cowork-runs.json)
...
[Tam boyutlu Claude uygulama kaydını aç](../../assets/marketing-claims-review/evidence/screenshots/04-treatment-claude-1-1920x1080.png)

![Pazarlama İddiaları İncelemesi skill'i yüklenmiş Claude Opus 4.8 uygulaması](../../assets/marketing-claims-review/evidence/screenshots/04-treatment-claude-1-1920x1080.png)

[Claude uygulama ham yanıtı](../../assets/marketing-claims-review/evidence/outputs/treatment-claude-1.txt) ·
[Auto uygulama ham yanıtı](../../assets/marketing-claims-review/evidence/outputs/treatment-auto-1.txt) ·
[Ön inceleme kaydı](../../assets/marketing-claims-review/evidence/metadata/preliminary-review.json)
...
```mermaid
flowchart LR
    A[FTC reklam yöntemi] --> D[Kısa ve atıflı yöntem kuralları]
    B[Kurgusal MCS-2.1 politikası] --> E[Karar ve yetki kuralları]
    C[Kurgusal pilot ve kampanya] --> F[Kanıt haritası ve senaryolar]
    D --> G[Altı dosyalı Cowork skill'i]
    E --> G
    F --> G
    G --> H[Oturum açılmış UX koşuları kaydedildi]
    G --> I[Kilitli resmî kıyaslama beklemede]
```

FTC yayınları iddia, açıklama ve tanıklık inceleme yöntemlerini besler. Lumena
politikasını belirlemez veya hukuki incelemenin yerini almaz. MCS-2.1 sentetiktir
ve demonun karar sınıflarını, iddia kurallarını, yayın sınırını ve yetkisini
belirler.
