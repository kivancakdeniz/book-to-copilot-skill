...
- **Investment Committee enterprise demo** — an OGL-attributed Green Book 2026
  ...
  evidence-led web page establish the first repeatable business demo.
- **Deterministic Cowork packaging** — `package_cowork_skill.py` builds root-level
  `.skill` archives with stable bytes and enforces Cowork file, compressed-size,
  uncompressed-size, frontmatter, hidden-file, and symbolic-link boundaries.
- **Reference security coverage** — the generated-skill scanner now recursively
  includes Cowork `references/` Markdown and rejects symbolic-link reference trees.
